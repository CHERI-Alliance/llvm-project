# RUN: not llvm-mc %s -triple=riscv32 -mattr=+a,+xcheri -riscv-no-aliases 2>&1 | FileCheck --check-prefixes=CHECK,CHECK-HYBRID %s
# RUN: not llvm-mc %s -triple=riscv64 -mattr=+a,+xcheri -riscv-no-aliases 2>&1 | FileCheck --check-prefixes=CHECK,CHECK-HYBRID %s
# RUN: not llvm-mc %s --defsym PURECAP=1 -triple=riscv32 -mattr=+a,+xcheri,+cap-mode -riscv-no-aliases 2>&1 | FileCheck %s
# RUN: not llvm-mc %s --defsym PURECAP=1 -triple=riscv64 -mattr=+a,+xcheri,+cap-mode -riscv-no-aliases 2>&1 | FileCheck %s
